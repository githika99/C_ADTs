...
private:
   ...
   //returns true if the subtrees rooted at N and P are identical.
   bool node_equals(const Node* N, const Node* P) const;

   //Helper Function for equals()
   //returns true if the subtrees rooted at N and P contain the same (key, val) pairs
   bool node_contains(const Node* N, Node* P) const;
